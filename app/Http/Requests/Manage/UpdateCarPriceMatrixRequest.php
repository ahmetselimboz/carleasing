...
class UpdateCarPriceMatrixRequest extends FormRequest
{
    public function authorize(): bool
    {
        $matrix = $this->route('price_matrix');

        return $matrix instanceof CarPriceMatrix && ($this->user()?->can('update', $matrix) ?? false);
    }
    ...
    {
        $matrix = $this->route('price_matrix');
        assert($matrix instanceof CarPriceMatrix);

        return [
            'car_package_id' => [
                'required',
                'exists:car_packages,id',
                Rule::unique('car_price_matrix', 'car_package_id')
                    ->ignore($matrix->id)
                    ->where(function ($query) use ($matrix) {
                        return $query
                            ->where('car_id', $matrix->car_id)
