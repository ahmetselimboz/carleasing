    ...
    <div class="fade-in space-y-6" x-data="{
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                ...
                <p class="text-slate-500 text-sm mt-1">Sitenin temel bilgilerini, görsellerini ve diğer tercihlerini buradan
                    yönetebilirsiniz.</p>
            </div>
        </div>
        ...
        <div class="card bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                <div class="border-b border-slate-200 px-4 lg:px-6">
                    <div class="flex gap-1 overflow-x-auto">
                        <button type="button" @click="activeTab = 'genel'"
                            :class="['px-4 py-3 flex items-center font-medium text-sm border-b-2 transition whitespace-nowrap',
                                activeTab === 'genel' ? 'admin-tab-active' :
                        ...
                        <button type="button" @click="activeTab = 'iletisim'"
                            :class="['px-4 py-3 flex items-center font-medium text-sm border-b-2 transition whitespace-nowrap',
                                activeTab === 'iletisim' ? 'admin-tab-active' :
                                'border-transparent text-slate-500 hover:text-slate-700'
                            ]">
                        ...
                        <button type="button" @click="activeTab = 'medya'"
                            :class="['px-4 py-3 flex items-center font-medium text-sm border-b-2 transition whitespace-nowrap',
                                activeTab === 'medya' ? 'admin-tab-active' :
                                'border-transparent text-slate-500 hover:text-slate-700'
                            ]">
                        ...
                        <button type="button" @click="activeTab = 'seo'"
                            :class="['px-4 py-3 flex items-center font-medium text-sm border-b-2 transition whitespace-nowrap',
                                activeTab === 'seo' ? 'admin-tab-active' :
                                'border-transparent text-slate-500 hover:text-slate-700'
                            ]">
                        ...
                        <button type="button" @click="activeTab = 'sistem'"
                            :class="['px-4 py-3 flex items-center font-medium text-sm border-b-2 transition whitespace-nowrap',
                                activeTab === 'sistem' ? 'admin-tab-active' :
                                'border-transparent text-slate-500 hover:text-slate-700'
                            ]">
                        ...
                        <button type="button" @click="activeTab = 'entegrasyon'"
                            :class="['px-4 py-3 flex items-center font-medium text-sm border-b-2 transition whitespace-nowrap',
                                activeTab === 'entegrasyon' ? 'admin-tab-active' :
                                'border-transparent text-slate-500 hover:text-slate-700'
                            ]">
                ...
                <div class="p-4 lg:p-6 space-y-8">
                    ...
                    <div x-show="activeTab === 'genel'" x-cloak class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="md:col-span-2">
                                <label for="title" class="block text-sm font-medium text-slate-700 mb-2">Site
                                    başlığı</label>
                                ...
                                @error('title')
                                    <p class="text-red-500 text-sm mt-1 flex items-center gap-1"><i
                                            class="ri-error-warning-line"></i> {{ $message }}</p>
                                @enderror
                            </div>
                            <div class="md:col-span-2">
                                <label for="description" class="block text-sm font-medium text-slate-700 mb-2">Kısa
                                    açıklama</label>
                                <textarea name="description" id="description" rows="3"
                                    ...
                                    maxlength="255" placeholder="Ana sayfada görünecek kısa tanıtım metni.">{{ old('description', $setting->description) }}</textarea>
                            </div>
                            <div class="md:col-span-2">
                                <label for="mb_site_copyright" class="block text-sm font-medium text-slate-700 mb-2">Telif
                                    yazısı</label>
                    ...
                    <div x-show="activeTab === 'iletisim'" x-cloak class="space-y-6">
                        <div class="rounded-xl border border-slate-100 bg-slate-50/80 p-5 space-y-4">
                            <h3 class="text-sm font-semibold text-slate-800 flex items-center gap-2"><i
                                    class="ri-contacts-line text-brand"></i> İletişim bilgileri</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label for="mb_contact_email"
                                        class="block text-sm font-medium text-slate-700 mb-2">E-posta</label>
                                    <div class="flex border border-slate-200 rounded-xl overflow-hidden bg-white">
                                        <span class="px-3 py-2.5 bg-slate-100 text-slate-600 border-r border-slate-200"><i
                                        ...
                                        <input type="email" name="magicbox[contact][email]" id="mb_contact_email"
                                            ...
                                            value="{{ old('magicbox.contact.email', data_get($m, 'contact.email')) }}"
                                ...
                                <div>
                                    <label for="mb_contact_phone"
                                        class="block text-sm font-medium text-slate-700 mb-2">Telefon</label>
                                    <div class="flex border border-slate-200 rounded-xl overflow-hidden bg-white">
                                        <span class="px-3 py-2.5 bg-slate-100 text-slate-600 border-r border-slate-200"><i
                                                class="ri-phone-line"></i></span>
                                        <input type="text" name="magicbox[contact][phone]" id="mb_contact_phone"
                                            class="flex-1 px-3 py-2.5 focus:ring-2 focus:ring-brand/20 focus:outline-none text-sm"
                                            value="{{ old('magicbox.contact.phone', data_get($m, 'contact.phone')) }}"
                                            placeholder="+90 …">
                                    </div>
                                </div>
                                <div class="md:col-span-2">
                                    <label for="mb_contact_address"
                                        class="block text-sm font-medium text-slate-700 mb-2">Adres</label>
                        ...
                        <div class="rounded-xl border border-slate-100 bg-white p-5 space-y-4 shadow-sm">
                            <h3 class="text-sm font-semibold text-slate-800 flex items-center gap-2"><i
                                    class="ri-links-line text-brand"></i> Sosyal medya hesapları</h3>
                            <p class="text-xs text-slate-500">Tam profil URL’lerini girin (örn. <code
                                    class="text-[11px] bg-slate-50 px-1 rounded border border-slate-200">https://instagram.com/kullanici</code>).
                                Boş bırakılan ağlar sitede gizlenir.</p>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach ([
            ...
            'tiktok' => ['TikTok', 'ri-tiktok-fill', 'https://tiktok.com/@…'],
                                    ...
                                    <div>
                                        <label for="mb_social_{{ $key }}"
                                            class="flex items-center gap-2 text-sm font-medium text-slate-700 mb-2">
                                            <i class="{{ $meta[1] }} text-brand"></i> {{ $meta[0] }}
                                        </label>
                                        <input type="text" name="magicbox[social][{{ $key }}]"
                                            id="mb_social_{{ $key }}" inputmode="url" autocomplete="url"
                                            class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand/20 text-sm"
            ...
            'placeholder_image' => ['label' => 'Varsayılan görsel (placeholder)', 'hint' => 'Listelerde görsel yoksa kullanılır.', 'icon' => 'ri-landscape-line'],
                            ...
                            <div class="rounded-2xl border border-slate-100 p-5 bg-slate-50/50">
                                <div class="flex flex-col lg:flex-row gap-6 lg:items-start">
                                    <div class="shrink-0">
                                        <div
                                            class="w-full max-w-[220px] aspect-[4/3] rounded-xl border-2 border-dashed border-slate-200 bg-white flex items-center justify-center overflow-hidden">
                                            <template x-if="previews['{{ $field }}']">
                                                <img :src="previews['{{ $field }}']" alt=""
                                    ...
                                    <div class="flex-1 min-w-0 space-y-3">
                                        <div>
                                            <label
                                                class="block text-sm font-medium text-slate-800">{{ $meta['label'] }}</label>
                                        ...
                                        @enderror
                                        @if ($setting->{$field})
                                            <p class="text-xs text-slate-500 break-all">Mevcut dosya: <code
                                                    class="bg-white px-1 rounded border border-slate-200">{{ $setting->{$field} }}</code>
                                            </p>
                                        @endif
                                    </div>
                                </div>
                    ...
                    <div x-show="activeTab === 'seo'" x-cloak class="space-y-6">
                        ...
                        @endphp
                        <div
                            class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-xl border border-slate-100 p-4 bg-white">
                            <div>
                            ...
                            <div class="flex items-center gap-3 shrink-0">
                                ...
                                <label class="inline-flex cursor-pointer items-center">
                                    ...
                                    <input type="checkbox" name="magicbox[seo][allow_indexing]" value="1"
                                        class="peer sr-only" @checked($allowIndexOn)>
                                    <span
                        ...
                        <div>
                            <label for="mb_seo_desc" class="block text-sm font-medium text-slate-700 mb-2">Varsayılan
                        ...
                        <div
                            class="flex items-center justify-between gap-4 rounded-xl border border-slate-100 p-4 bg-white">
                            <div>
                                <p class="text-sm font-medium text-slate-800">Sosyal medya paylaşım ayarı</p>
                                <p class="text-xs text-slate-500 mt-0.5">Açık olduğunda paylaşımlarda varsayılan görsel ve
                                    metin kullanılır.</p>
                            </div>
                            <label class="inline-flex cursor-pointer items-center gap-3 shrink-0">
                                <input type="checkbox" name="magicbox[features][open_graph_default]" value="1"
                                    class="peer sr-only" @checked(old('magicbox.features.open_graph_default', (bool) data_get($m, 'features.open_graph_default')))>
                                <span
                                    class="relative inline-flex h-6 w-12 shrink-0 rounded-full bg-slate-200 transition-colors peer-checked:bg-brand-solid after:absolute after:left-1 after:top-1 after:h-4 after:w-4 after:rounded-full after:bg-white after:shadow after:transition-transform after:content-[''] peer-checked:after:translate-x-6"></span>
                            </label>
                        </div>
                    ...
                    <div x-show="activeTab === 'sistem'" x-cloak class="space-y-8">
                        <div class="rounded-xl border border-slate-200 p-5 space-y-4">
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500 mt-1">Açık olduğunda ziyaretçiler bakım ekranı görür.
                                    </p>
                                </div>
                                <label class="inline-flex cursor-pointer items-center gap-3 shrink-0">
                            ...
                            <div>
                                <label for="mb_maint_msg" class="block text-sm font-medium text-slate-700 mb-2">Bakım
                                    mesajı</label>
                            ...
                            <div class="max-w-md">
                                <label for="mb_maint_end" class="block text-sm font-medium text-slate-700 mb-2">Tahmini
                                    bitiş (isteğe bağlı)</label>
                                <input type="datetime-local" name="magicbox[maintenance][estimated_end]"
                                    id="mb_maint_end"
                                    class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand/20 text-sm"
                        ...
                        <div class="space-y-3">
                            ...
                            <p class="text-xs text-slate-500">Bu seçenekleri açıp kapatarak site davranışını kolayca
                                yönetebilirsiniz.</p>
            ...
            'allow_registration' => ['Kullanıcı kaydına izin ver', 'ri-user-add-line'],
                                ...
                                <div
                                    ...
                                    <div class="flex items-start gap-3 min-w-0">
                                        <span class="mt-0.5 text-brand"><i
                                                class="{{ $info[1] }} text-lg"></i></span>
                                        <div>
                                            <p class="text-sm font-medium text-slate-800">{{ $info[0] }}</p>
                                        </div>
                                    </div>
                                    <label class="inline-flex cursor-pointer items-center shrink-0">
                                        <input type="checkbox" name="magicbox[features][{{ $feat }}]"
                                            value="1" class="peer sr-only" @checked(old('magicbox.features.' . $feat, (bool) data_get($m, 'features.' . $feat)))>
                                        <span
                                            class="relative inline-flex h-6 w-12 rounded-full bg-slate-200 transition-colors peer-checked:bg-brand-solid after:absolute after:left-1 after:top-1 after:h-4 after:w-4 after:rounded-full after:bg-white after:shadow after:transition-transform after:content-[''] peer-checked:after:translate-x-6"></span>
                                    </label>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div x-show="activeTab === 'entegrasyon'" x-cloak class="space-y-6">
                        ...
                        <div class="rounded-xl border border-slate-100 bg-white p-5 space-y-4 shadow-sm">
                            <h3 class="text-sm font-semibold text-slate-800 flex items-center gap-2"><i
                                    class="ri-bar-chart-line text-brand"></i> Ziyaret ölçümü (GA / GTM)</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label for="ga_id" class="block text-sm font-medium text-slate-700 mb-2">Google
                                        Analytics kodu</label>
                                    <input type="text" name="magicbox[integrations][google_analytics_id]"
                                        id="ga_id"
                                ...
                                <div>
                                    <label for="gtm_id" class="block text-sm font-medium text-slate-700 mb-2">Google Tag
                                        Manager</label>
                                    <input type="text" name="magicbox[integrations][google_tag_manager_id]"
                                        id="gtm_id"
                                        class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand/20 text-sm font-mono"
                            ...
                            <div>
                                <label for="mb_ga_track" class="block text-sm font-medium text-slate-700 mb-2">Google
                                    Analytics kodu</label>
                                <p class="text-xs text-slate-500 mb-2">Google'dan aldığınız takip kodunu buraya
                        ...
                        <div class="rounded-xl border border-slate-100 bg-white p-5 space-y-4 shadow-sm">
                            <h3 class="text-sm font-semibold text-slate-800 flex items-center gap-2"><i
                                    class="ri-google-fill text-brand"></i> Google bağlantı bilgileri</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label for="mb_g_client_id"
                                        class="block text-sm font-medium text-slate-700 mb-2">Google uygulama
                                        kimliği</label>
                                    <input type="text" name="magicbox[google][oauth_client_id]" id="mb_g_client_id"
                                        class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm font-mono"
                                        value="{{ old('magicbox.google.oauth_client_id', data_get($m, 'google.oauth_client_id')) }}"
                                        autocomplete="off">
                                ...
                                <div>
                                    <label for="mb_g_client_secret"
                                        class="block text-sm font-medium text-slate-700 mb-2">Google gizli anahtar</label>
                                    <input type="password" name="magicbox[google][oauth_client_secret]"
                                        id="mb_g_client_secret"
                                        class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm font-mono"
                                        value="{{ old('magicbox.google.oauth_client_secret') }}"
                                ...
                                <div class="md:col-span-2">
                                    <label for="mb_g_redirect"
                                        class="block text-sm font-medium text-slate-700 mb-2">Google yönlendirme
                                        adresi</label>
                                    <input type="text" name="magicbox[google][oauth_redirect_url]" id="mb_g_redirect"
                                        inputmode="url"
                                ...
                                <div class="md:col-span-2">
                                    <label for="mb_g_property"
                                        class="block text-sm font-medium text-slate-700 mb-2">Google hesap kimliği</label>
                        ...
                        <div class="rounded-xl border border-slate-100 bg-white p-5 space-y-4 shadow-sm">
                            <h3 class="text-sm font-semibold text-slate-800 flex items-center gap-2"><i
                                    class="ri-code-s-slash-line text-brand"></i> Özel kod alanları</h3>
                            <div class="space-y-4">
                                <div>
                                    <label for="mb_inj_head"
                                        class="block text-sm font-medium text-slate-700 mb-2">&lt;head&gt; alanı</label>
                                ...
                                <div>
                                    <label for="mb_inj_body"
                                        class="block text-sm font-medium text-slate-700 mb-2">&lt;body&gt; açılış
                                        (üst)</label>
                                ...
                                <div>
                                    <label for="mb_inj_footer"
                                        class="block text-sm font-medium text-slate-700 mb-2">Footer / &lt;body&gt; kapanış
                                        öncesi</label>
                        ...
                        <div class="rounded-xl border border-slate-100 bg-white p-5 space-y-4 shadow-sm">
                            <h3 class="text-sm font-semibold text-slate-800 flex items-center gap-2"><i
                                    class="ri-file-text-line text-brand"></i> ads.txt</h3>
                            <p class="text-xs text-slate-500">Reklam altyapısı kullanıyorsanız bu alana `ads.txt` içeriğini
                                ekleyebilirsiniz.</p>
                        ...
                        <div class="rounded-xl border border-slate-100 bg-white p-5 space-y-4 shadow-sm">
                            <h3 class="text-sm font-semibold text-slate-800 flex items-center gap-2"><i
                                    class="ri-mail-send-line text-brand"></i> E-posta gönderim ayarları (SMTP)</h3>
                            <p class="text-xs text-slate-500">
                                Bu alanlar sayesinde sistem, kullanıcılara ve yönetim ekibine otomatik e-posta gönderir.
                                Alan adları teknik olarak İngilizce kalır; açıklamalar Türkçe verilmiştir.
                            </p>
                            <div
                                class="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600 space-y-1">
                                <p><strong>MAIL_MAILER</strong>: Gönderim yöntemi. Genelde <code
                                        class="bg-white px-1 rounded border border-slate-200">smtp</code> kullanılır.</p>
                                <p><strong>MAIL_HOST</strong> / <strong>MAIL_PORT</strong>: E-posta sağlayıcınızın sunucu
                                    adresi ve port numarası.</p>
                                <p><strong>MAIL_USERNAME</strong> / <strong>MAIL_PASSWORD</strong>: SMTP giriş bilgileri.
                                </p>
                                <p><strong>MAIL_FROM_ADDRESS</strong> / <strong>MAIL_FROM_NAME</strong>: Alıcıda görünen
                                    gönderen e-posta ve ad.</p>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label for="mb_smtp_host" class="block text-sm font-medium text-slate-700 mb-2">SMTP
                                        sunucu</label>
                                    <input type="text" name="magicbox[mail][smtp_host]" id="mb_smtp_host"
                                        class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm"
                                        value="{{ old('magicbox.mail.smtp_host', data_get($m, 'mail.smtp_host')) }}"
                                        placeholder="smtp.mailprovider.com">
                                </div>
                                <div>
                                    <label for="mb_smtp_port"
                                        class="block text-sm font-medium text-slate-700 mb-2">Port</label>
                                    <input type="text" name="magicbox[mail][smtp_port]" id="mb_smtp_port"
                                        class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm font-mono"
                                        value="{{ old('magicbox.mail.smtp_port', data_get($m, 'mail.smtp_port')) }}"
                                        placeholder="587">
                                </div>
                                <div>
                                    <label for="mb_smtp_user"
                                        class="block text-sm font-medium text-slate-700 mb-2">Kullanıcı adı</label>
                                    <input type="text" name="magicbox[mail][smtp_username]" id="mb_smtp_user"
                                        autocomplete="off"
                                        class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm"
                                        value="{{ old('magicbox.mail.smtp_username', data_get($m, 'mail.smtp_username')) }}">
                                </div>
                                <div>
                                    <label for="mb_smtp_pass"
                                        class="block text-sm font-medium text-slate-700 mb-2">Şifre</label>
                                    <input type="password" name="magicbox[mail][smtp_password]" id="mb_smtp_pass"
                                        autocomplete="new-password"
                                        class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm font-mono"
                                        value="{{ old('magicbox.mail.smtp_password') }}"
                                        placeholder="{{ filled(data_get($m, 'mail.smtp_password')) ? 'Değiştirmek için yeni şifre' : 'SMTP şifresi' }}">
                                </div>
                                <div>
                                    <label for="mb_smtp_enc"
                                        class="block text-sm font-medium text-slate-700 mb-2">Şifreleme</label>
                                ...
                                <div>
                                    <label for="mb_mail_from"
                                        class="block text-sm font-medium text-slate-700 mb-2">Gönderen e-posta</label>
                                    <input type="email" name="magicbox[mail][from_address]" id="mb_mail_from"
                                        class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm"
                                        value="{{ old('magicbox.mail.from_address', data_get($m, 'mail.from_address')) }}"
                                        placeholder="noreply@site.com">
                                </div>
                                <div class="md:col-span-2">
                                    <label for="mb_mail_from_name"
                                        class="block text-sm font-medium text-slate-700 mb-2">Gönderen adı</label>
                                    <input type="text" name="magicbox[mail][from_name]" id="mb_mail_from_name"
                                        class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm"
                                        value="{{ old('magicbox.mail.from_name', data_get($m, 'mail.from_name')) }}"
                        ...
                        <div class="rounded-xl border border-slate-100 bg-white p-5 space-y-4 shadow-sm">
                            <h3 class="text-sm font-semibold text-slate-800 flex items-center gap-2"><i
                                    class="ri-notification-3-line text-brand"></i> Talep bildirim e-posta ayarları</h3>
                            <p class="text-xs text-slate-500">
                                Kiralama Talebi, İletişim Mesajı ve Geri Arama Talebi oluştuğunda hem talep sahibine hem de
                                iç ekibe e-posta atılmasını yönetir.
                            </p>
                            <div
                                class="rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600 space-y-1">
                                <p><strong>LEAD_MAIL_ENABLED=true</strong>: Otomatik talep e-postalarını aç/kapat.</p>
                                <p><strong>LEAD_MAIL_ADMIN_ROLES=super_admin,admin,customer_service</strong>: Kimlere "yeni
                                    talep geldi" maili gideceğini belirler.</p>
                                <p><strong>LEAD_MAIL_FORCE_TO</strong>: Test ortamı için tüm mailleri tek adrese
                                    yönlendirir. Boş ise normal dağıtım yapılır.</p>
                            </div>
                            <div class="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3">
                                Not: Bu değerler <code class="bg-white px-1 rounded border border-amber-200">.env</code>
                                üzerinden yönetilir.
                                Ayarları güncelledikten sonra <code
                                    class="bg-white px-1 rounded border border-amber-200">php artisan config:clear</code>
                                çalıştırıp
                                kuyruk için <code class="bg-white px-1 rounded border border-amber-200">php artisan
                                    queue:work</code> açık tutun.
                            </div>
                        </div>
                    </div>
                    ...
                    @if ($errors->any())
                        <div class="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800" role="alert">
                            <p class="font-medium flex items-center gap-2"><i class="ri-error-warning-line"></i> Lütfen
                                formu kontrol edin.</p>
