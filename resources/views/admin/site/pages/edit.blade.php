...
@section('content')
    <div class="fade-in space-y-6">
        <div>
            ...
            <h2 class="text-2xl font-bold text-slate-800">Sayfa düzenle</h2>
            <p class="text-slate-500 text-sm mt-1">Sayfa bilgilerini ve içeriğini güncelleyebilirsiniz.</p>
        </div>
            ...
            <form method="POST" action="{{ route('pages.update', $page) }}" class="space-y-5">
                    ...
                    <select name="page_category_id" id="page_category_id"
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}"
                                @selected((int) old('page_category_id', $page->page_category_id) === $category->id)>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="7"
                        class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand/20 focus:border-brand transition-soft @error('description') border-red-500 @enderror">{{ old('description', $page->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
