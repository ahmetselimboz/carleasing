...
@section('content')
    <div class="fade-in space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-3xl font-bold text-slate-800">Sayfalar</h2>
                <p class="text-slate-500 text-sm mt-1">Site içerik sayfalarını bu alandan yönetebilirsiniz.</p>
            </div>
        ...
        <div class="card bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <form method="GET" class="px-4 pt-4 grid grid-cols-1 md:grid-cols-5 gap-3">
                <input type="text" name="q" value="{{ request('q') }}" placeholder="Başlık, slug veya açıklama ara..."
                    class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl">
                <select name="category_id" class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl">
                ...
                <select name="sort" class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl">
                    ...
                    <option value="title_asc" @selected(request('sort') === 'title_asc')>Başlık: A-Z</option>
                    <option value="title_desc" @selected(request('sort') === 'title_desc')>Başlık: Z-A</option>
                </select>
                <button type="submit"
                        ...
                        @forelse ($pages as $page)
                            <tr class="hover:bg-slate-50/80">
                                <td class="px-4 py-3">
                                    ...
                                    <p class="text-xs text-slate-400 mt-0.5">#{{ $page->id }}</p>
                                </td>
                                <td class="px-4 py-3 text-slate-600">{{ $page->category?->name ?: '-' }}</td>
                                <td class="px-4 py-3 text-slate-600 font-mono text-xs">{{ $page->slug }}</td>
                                <td class="px-4 py-3">
                                    @if ($page->is_active)
                                        <span
