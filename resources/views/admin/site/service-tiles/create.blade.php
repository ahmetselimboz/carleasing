...
@section('content')
    @php($iconOptions = config('admin-icons.service_tile_icons', []))
    <div class="fade-in space-y-8">
        <div>
            ...
            <h2 class="text-2xl font-bold text-slate-800">Yeni hizmet kutusu</h2>
        </div>
        <div class="card bg-white rounded-2xl border border-slate-100 shadow-sm p-6 lg:p-8">
            <form method="POST" action="{{ route('home-service-tiles.store') }}" enctype="multipart/form-data" class="space-y-5">
                @csrf
                <div class="grid grid-cols-1 xl:grid-cols-2 gap-5">
                    ...
                    <input type="hidden" name="icon" id="icon" value="{{ old('icon', 'ri-flight-takeoff-line') }}">
                    <div class="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                        <span id="icon-preview" class="inline-flex items-center justify-center h-12 w-12 rounded-xl bg-white border border-slate-200 text-2xl text-slate-700"><i class="{{ old('icon', 'ri-flight-takeoff-line') }}"></i></span>
                        <div class="flex-1 min-w-[220px]">
                            <p class="text-sm font-medium text-slate-700">Seçilen ikon</p>
                            <p id="icon-label" class="text-xs text-slate-500 font-mono">{{ old('icon', 'ri-flight-takeoff-line') }}</p>
                        </div>
                ...
                <div class="grid grid-cols-1 xl:grid-cols-2 gap-5">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl">
                    </div>
                    <div>
                        <label for="link_url" class="block text-sm font-medium text-slate-700 mb-2">İsteğe bağlı bağlantı</label>
                        <input type="url" name="link_url" id="link_url" value="{{ old('link_url') }}" placeholder="https://" class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl">
                    </div>
                </div>
                <div class="grid grid-cols-1 xl:grid-cols-2 gap-5">
                    <div>
                    <label for="description" class="block text-sm font-medium text-slate-700 mb-2">Açıklama</label>
                    <textarea name="description" id="description" rows="3" class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl">{{ old('description') }}</textarea>
                    </div>
                    <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Görsel</label>
