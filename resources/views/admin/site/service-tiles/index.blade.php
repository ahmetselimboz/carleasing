...
@section('content')
    <div class="fade-in space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-3xl font-bold text-slate-800">Hizmet kutuları</h2>
                <p class="text-slate-500 text-sm mt-1">Anasayfadaki hizmet kartlarını buradan düzenleyebilirsiniz.</p>
            </div>
            @can('create', App\Models\HomeServiceTile::class)
                <a href="{{ route('home-service-tiles.create') }}"
        ...
        <div class="card bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <form method="GET" class="px-4 pt-4 grid grid-cols-1 md:grid-cols-5 gap-3">
                <input type="text" name="q" value="{{ request('q') }}" placeholder="Başlık/açıklama ara..."
            ...
            <div class="space-y-4">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody id="tiles-table-body" class="divide-y divide-slate-100">
                            @forelse ($tiles as $t)
                                <tr class="tile-row hover:bg-slate-50/80" draggable="true" data-id="{{ $t->id }}">
                                    <td class="px-4 py-3 text-slate-400 cursor-move">
                                        <span class="inline-flex items-center gap-1"><i class="ri-draggable"></i> Taşı</span>
                                    </td>
                                    <td class="px-4 py-3"><span class="tile-order">{{ $loop->iteration }}</span></td>
                                    <td class="px-4 py-3 font-medium">{{ $t->title ?: '—' }}</td>
                                    <td class="px-4 py-3 text-xs font-mono text-slate-500">{{ $t->icon ?: '—' }}</td>
                                    <td class="px-4 py-3 text-right">
                                        @can('view', $t)
                                            <a href="{{ route('home-service-tiles.edit', $t) }}" class="text-xs font-medium text-brand hover:underline">Düzenle</a>
                                        @endcan
                                        @can('delete', $t)
                                            <form method="POST" action="{{ route('home-service-tiles.destroy', $t) }}" class="inline ml-2"
            ...
            <form id="tiles-reorder-form" method="POST" action="{{ route('home-service-tiles.reorder') }}" class="hidden">
                @csrf
                <div id="tiles-reorder-inputs"></div>
            </form>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            var body = document.getElementById('tiles-table-body');
            if (!body) return;
            var form = document.getElementById('tiles-reorder-form');
            var inputs = document.getElementById('tiles-reorder-inputs');
            var dragged = null;
            var moved = false;

            function refreshNumbers() {
                var rows = body.querySelectorAll('.tile-row');
                rows.forEach(function(row, index) {
                    var slot = row.querySelector('.tile-order');
                    if (slot) slot.textContent = String(index + 1);
                });
            }

            body.addEventListener('dragstart', function(e) {
                var row = e.target.closest('.tile-row');
                if (!row) return;
                dragged = row;
                moved = false;
