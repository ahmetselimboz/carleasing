...
@section('content')
    <div class="fade-in space-y-6">
        <div>
            ...
            <h2 class="text-2xl font-bold text-slate-800">Referans düzenle</h2>
            <p class="text-slate-500 text-sm mt-1">Marka bilgilerini ve görünüm ayarlarını güncelleyebilirsiniz.</p>
        </div>
            ...
            <form method="POST" action="{{ route('references.update', $reference) }}" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $reference->name) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $reference->title) }}"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="url" name="link" id="link" value="{{ old('link', $reference->link) }}"
                    ...
                    <div>
                        <label for="image" class="block text-sm font-medium text-slate-700 mb-2">Yeni logo görseli</label>
                ...
                @if ($reference->image)
                    <div>
                        <p class="block text-sm font-medium text-slate-700 mb-2">Mevcut logo</p>
                ...
                <div>
                    ...
                    <textarea name="detail" id="detail" rows="4"
                        class="input-base w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand/20 focus:border-brand transition-soft @error('detail') border-red-500 @enderror">{{ old('detail', $reference->detail) }}</textarea>
                    @error('detail')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
